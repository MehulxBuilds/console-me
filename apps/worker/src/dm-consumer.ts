...
import { geDMChatCache } from "@repo/cache";
...
export class DMConsumer {
    ...
    async start() {
        ...
        try {
            ...
            console.log("Kafka DM_Message Consumer: Connected and listening...");
            ...
            await this.consumer.run({
                ...
                eachBatch: async ({
                    ...
                    let currentBatch: (DMChatMessage & { offset: string })[] = [];
                    ...
                    for (const message of messages) {
                        ...
                        try {
                            ...
                            const content = JSON.parse(rawValue) as DMChatMessage;
                            currentBatch.push({
                                ...content,
    ...
    private async processBatchItems(
        messages: (DMChatMessage & { offset: string })[],
    ) {
        ...
        try {
            ...
            // const postCache = geDMChatCache();
            // const invalidatedKeys = new Set<string>();

            // for (const msg of posts) {
            //     // Invalidate the creator's profile posts cache
            //     const key = `creator:${msg?.senderId}:posts:initial`;
            //     if (!invalidatedKeys.has(key)) {
            //         await postCache.invalidateDMChat(key);
            ...
            try {
                for (const msg of messages) {
                    ...
                    await producer.publishDMChatMessage(originalMessage);
                }
                console.log(
