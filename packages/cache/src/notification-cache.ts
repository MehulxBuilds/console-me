...
export class NotificationCache {
    private redis: Redis;
    // 5 minutes TTL for cached Notification
    ...
    async addNotification(key: string, messages: any[]): Promise<void> {
    ...
    async getNotification(key: string): Promise<any[] | null> {
        try {
    ...
    async invalidateNotification(key: string): Promise<void> {
        try {
