...
export class DMChatCache {
    private redis: Redis;
    // 5 minutes TTL for cached DMChat
    ...
    async addDMChat(key: string, messages: any[]): Promise<void> {
    ...
    async getDMChat(key: string): Promise<any[] | null> {
        try {
    ...
    async invalidateDMChat(key: string): Promise<void> {
        try {
